...
class Xls extends BaseWriter
{
    ...
    private function writeDocumentSummaryInformation()
    {
        ...
        foreach ($dataSection as $dataProp) {
            ...
            } elseif ($dataProp['type']['data'] == 3) {
                // 4 byte signed integer
                $dataSection_Content .= pack('V', $dataProp['data']['data']);
                $dataSection_Content_Offset += 4 + 4;
            } elseif ($dataProp['type']['data'] == 11) {
                // Boolean
    ...
    private function writeSummaryInformation()
    {
        ...
        if ($this->spreadsheet->getProperties()->getCreated()) {
            ...
            $dataSection[] = array('summary' => array('pack' => 'V', 'data' => 12), 'offset' => array('pack' => 'V'), 'type' => array('pack' => 'V', 'data' => 64), 'data' => array('data' => OLE::localDateToOLE($dataProp)));
            ++$dataSection_NumProps;
        }
        //    Modified Date/Time
        if ($this->spreadsheet->getProperties()->getModified()) {
            $dataProp = $this->spreadsheet->getProperties()->getModified();
            $dataSection[] = array('summary' => array('pack' => 'V', 'data' => 13), 'offset' => array('pack' => 'V'), 'type' => array('pack' => 'V', 'data' => 64), 'data' => array('data' => OLE::localDateToOLE($dataProp)));
            ++$dataSection_NumProps;
        }
        //    Security
