...
class Xls extends BaseReader
{
    ...
    private function readTextObject()
    {
        ...
        $this->textObjects[$this->textObjRef] = array('text' => $textStr, 'format' => substr($text['recordData'], $text['spliceOffsets'][1], $cbRuns), 'alignment' => $grbitOpts, 'rotation' => $rot);
    }
    /**
     * Read BOF.
     */
...
';
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isItalic = (2 & self::getUInt2d($recordData, 2)) >> 1;
            if ($isItalic) {
                $objFont->setItalic(true);
            }
            // bit: 2; mask 0x0004; underlined (redundant in BIFF5-BIFF8)
            // bit: 3; mask 0x0008; strikethrough
            $isStrike = (8 & self::getUInt2d($recordData, 2)) >> 3;
            if ($isStrike) {
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (isset($this->formats[$numberFormatIndex])) {
                ...
                $numberFormat = array('formatCode' => $this->formats[$numberFormatIndex]);
            } elseif (($code = NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
                // then we have built-in format code
                $numberFormat = array('formatCode' => $code);
            } else {
                // we set the general format code
                $numberFormat = array('formatCode' => 'General');
            }
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($bordersLeftStyle = Xls\Style\Border::lookup((15 & self::getInt4d($recordData, 10)) >> 0)) {
                    $objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle = Xls\Style\Border::lookup((240 & self::getInt4d($recordData, 10)) >> 4)) {
                    $objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle = Xls\Style\Border::lookup((3840 & self::getInt4d($recordData, 10)) >> 8)) {
                    $objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
                }
                // bit: 15-12; mask: 0x0000F000; bottom style
                if ($bordersBottomStyle = Xls\Style\Border::lookup((61440 & self::getInt4d($recordData, 10)) >> 12)) {
                ...
                if ($bordersDiagonalStyle = Xls\Style\Border::lookup((31457280 & self::getInt4d($recordData, 14)) >> 21)) {
                    $objStyle->getBorders()->getDiagonal()->setBorderStyle($bordersDiagonalStyle);
                }
                // bit: 31-26; mask: 0xFC000000 fill pattern
                if ($fillType = Xls\Style\FillPattern::lookup((4227858432.0 & self::getInt4d($recordData, 14)) >> 26)) {
                    $objStyle->getFill()->setFillType($fillType);
                }
                // offset: 18; size: 2; pattern and background colour
                // bit: 6-0; mask: 0x007F; color index for pattern color
                $objStyle->getFill()->startcolorIndex = (127 & self::getUInt2d($recordData, 18)) >> 0;
                // bit: 13-7; mask: 0x3F80; color index for pattern background
                $objStyle->getFill()->endcolorIndex = (16256 & self::getUInt2d($recordData, 18)) >> 7;
            } else {
                // BIFF5
                ...
                $objStyle->getFill()->setFillType(Xls\Style\FillPattern::lookup((4128768 & $borderAndBackground) >> 16));
                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()->getBottom()->setBorderStyle(Xls\Style\Border::lookup((29360128 & $borderAndBackground) >> 22));
                // bit: 31-25; mask: 0xFE000000; bottom line color
                $objStyle->getBorders()->getBottom()->colorIndex = (0.0 & $borderAndBackground) >> 25;
                // offset: 12; size: 4; cell border lines
                $borderLines = self::getInt4d($recordData, 12);
                // bit: 2-0; mask: 0x00000007; top line style
                $objStyle->getBorders()->getTop()->setBorderStyle(Xls\Style\Border::lookup((7 & $borderLines) >> 0));
                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()->setBorderStyle(Xls\Style\Border::lookup((56 & $borderLines) >> 3));
                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()->setBorderStyle(Xls\Style\Border::lookup((448 & $borderLines) >> 6));
    ...
    private function readExternalBook()
    {
        ...
        } elseif (substr($recordData, 2, 2) == pack('CC', 1, 4)) {
            // internal reference
            // offset: 0; size: 2; number of sheet in this document
            // offset: 2; size: 2; 0x01 0x04
            $this->externalBooks[] = array('type' => 'internal');
        } elseif (substr($recordData, 0, 4) == pack('vCC', 1, 1, 58)) {
            // add-in function
            // offset: 0; size: 2; 0x0001
            $this->externalBooks[] = array('type' => 'addInFunction');
        } elseif (substr($recordData, 0, 2) == pack('v', 0)) {
            // DDE links, OLE links
    ...
    private function readExternSheet()
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            $nm = self::getUInt2d($recordData, 0);
            for ($i = 0; $i < $nm; ++$i) {
                $this->ref[] = array('externalBookIndex' => self::getUInt2d($recordData, 2 + 6 * $i), 'firstSheetIndex' => self::getUInt2d($recordData, 4 + 6 * $i), 'lastSheetIndex' => self::getUInt2d($recordData, 6 + 6 * $i));
            }
        }
    }
    /**
    ...
    private function readSheetPr()
    {
        ...
        $isSummaryBelow = (64 & self::getUInt2d($recordData, 0)) >> 6;
        $this->phpSheet->setShowSummaryBelow($isSummaryBelow);
        // bit: 7; mask: 0x0080; 0 = outline buttons left of outline group
        $isSummaryRight = (128 & self::getUInt2d($recordData, 0)) >> 7;
        $this->phpSheet->setShowSummaryRight($isSummaryRight);
        // bit: 8; mask: 0x100; 0 = scale printout in percent, 1 = fit printout to number of pages
        // this corresponds to radio button setting in page setup dialog in Excel
        $this->isFitToPages = (bool) ((256 & self::getUInt2d($recordData, 0)) >> 8);
    }
    /**
    ...
    private function readPageSetup()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isPortrait = (2 & self::getUInt2d($recordData, 10)) >> 1;
            // bit: 2; mask: 0x0004; 1= paper size, scaling factor, paper orient. not init
            // when this bit is set, do not use flags for those properties
            $isNotInit = (4 & self::getUInt2d($recordData, 10)) >> 2;
            if (!$isNotInit) {
    ...
    private function readProtect()
    {
        ...
        $bool = (1 & self::getUInt2d($recordData, 0)) >> 0;
    ...
    private function readColInfo()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isHidden = (1 & self::getUInt2d($recordData, 8)) >> 0;
            // bit: 10-8; mask: 0x0700; outline level of the columns (0 = no outline)
            $level = (1792 & self::getUInt2d($recordData, 8)) >> 8;
            // bit: 12; mask: 0x1000; 1 = collapsed
            $isCollapsed = (4096 & self::getUInt2d($recordData, 8)) >> 12;
    ...
    private function readRow()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $height = (32767 & self::getUInt2d($recordData, 6)) >> 0;
            // bit: 15: mask: 0x8000; 0 = row has custom height; 1= row has default height
            $useDefaultHeight = (32768 & self::getUInt2d($recordData, 6)) >> 15;
            if (!$useDefaultHeight) {
    ...
    private function readHyperLink()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $isFileLinkOrUrl = (1 & self::getUInt2d($recordData, 28)) >> 0;
            // bit: 1; mask: 0x00000002; 0 = relative path, 1 = absolute path or URL
            $isAbsPathOrUrl = (1 & self::getUInt2d($recordData, 28)) >> 1;
            // bit: 2 (and 4); mask: 0x00000014; 0 = no description
            $hasDesc = (20 & self::getUInt2d($recordData, 28)) >> 2;
            // bit: 3; mask: 0x00000008; 0 = no text, 1 = has text
            $hasText = (8 & self::getUInt2d($recordData, 28)) >> 3;
            // bit: 7; mask: 0x00000080; 0 = no target frame, 1 = has target frame
            $hasFrame = (128 & self::getUInt2d($recordData, 28)) >> 7;
            // bit: 8; mask: 0x00000100; 0 = file link or URL, 1 = UNC path (inc. server name)
            $isUNC = (256 & self::getUInt2d($recordData, 28)) >> 8;
            // offset within record data
            $offset = 32;
            if ($hasDesc) {
            ...
            } elseif (ord($recordData[$offset]) == 3) {
                $hyperlinkType = 'local';
            } elseif (ord($recordData[$offset]) == 224) {
                $hyperlinkType = 'URL';
            }
...
', $token['data']['spacecount']);
    ...
    private function readBIFF8CellAddress($cellAddressStructure)
    {
        ...
        $column = Coordinate::stringFromColumnIndex((255 & self::getUInt2d($cellAddressStructure, 2)) + 1);
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(16384 & self::getUInt2d($cellAddressStructure, 2))) {
            $column = '$' . $column;
        }
        // bit: 15; mask 0x8000; (1 = relative row index, 0 = absolute row index)
        if (!(32768 & self::getUInt2d($cellAddressStructure, 2))) {
            $row = '$' . $row;
        }
        return $column . $row;
    }
    ...
    private function readBIFF8CellAddressB($cellAddressStructure, $baseCell = 'A1')
    {
        ...
        if (!(16384 & self::getUInt2d($cellAddressStructure, 2))) {
            // offset: 2; size: 2; index to column or column offset + relative flags
            // bit: 7-0; mask 0x00FF; column index
            $colIndex = 255 & self::getUInt2d($cellAddressStructure, 2);
            $column = Coordinate::stringFromColumnIndex($colIndex + 1);
            $column = '$' . $column;
        } else {
            // offset: 2; size: 2; index to column or column offset + relative flags
            // bit: 7-0; mask 0x00FF; column index
            $relativeColIndex = 255 & self::getInt2d($cellAddressStructure, 2);
    ...
    private function readBIFF8CellRangeAddress($subData)
    {
        ...
        // bit: 7-0; mask 0x00FF; column index
        $fc = Coordinate::stringFromColumnIndex((255 & self::getUInt2d($subData, 4)) + 1);
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(16384 & self::getUInt2d($subData, 4))) {
            $fc = '$' . $fc;
        }
        // bit: 15; mask 0x8000; (1 = relative row index, 0 = absolute row index)
        if (!(32768 & self::getUInt2d($subData, 4))) {
            $fr = '$' . $fr;
        }
        // offset: 6; size: 2; index to last column or column offset + relative flags
        // bit: 7-0; mask 0x00FF; column index
        $lc = Coordinate::stringFromColumnIndex((255 & self::getUInt2d($subData, 6)) + 1);
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(16384 & self::getUInt2d($subData, 6))) {
            $lc = '$' . $lc;
        }
        // bit: 15; mask 0x8000; (1 = relative row index, 0 = absolute row index)
        if (!(32768 & self::getUInt2d($subData, 6))) {
            $lr = '$' . $lr;
        }
        return "{$fc}{$fr}:{$lc}{$lr}";
    }
    ...
    private function readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
    {
        ...
        if (!(16384 & self::getUInt2d($subData, 4))) {
            ...
            // bit: 7-0; mask 0x00FF; column index
            $fcIndex = 255 & self::getUInt2d($subData, 4);
            ...
            // offset: 4; size: 2; first column with relative/absolute flags
            // bit: 7-0; mask 0x00FF; column index
            $relativeFcIndex = 255 & self::getInt2d($subData, 4);
        ...
        if (!(16384 & self::getUInt2d($subData, 6))) {
            ...
            // bit: 7-0; mask 0x00FF; column index
            $lcIndex = 255 & self::getUInt2d($subData, 6);
            ...
            // offset: 4; size: 2; first column with relative/absolute flags
            // bit: 7-0; mask 0x00FF; column index
            $relativeLcIndex = 255 & self::getInt2d($subData, 4);
        ...
        if (isset($this->ref[$index])) {
            ...
            switch ($type) {
                case 'internal':
                    ...
                    if ($this->ref[$index]['firstSheetIndex'] == 65535 or $this->ref[$index]['lastSheetIndex'] == 65535) {
    ...
    {
        return ($tmp = unpack('s', $data[$pos] . $data[$pos + 1])) ? $tmp[1] : $tmp[1];
    }
    /**
